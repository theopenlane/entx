...
type Annotations struct {
	...
	// HasSystemOwnedField indicates if the edge being checked can be system owned
	HasSystemOwnedField bool `yaml:"HasSystemOwnedField,omitempty"`
}
...
func (e Extension) Hook() gen.Hook {
	return func(next gen.Generator) gen.Generator {
		return gen.GenerateFunc(func(g *gen.Graph) error {
			...
			systemOwnedByType := make(map[string]bool, len(g.Nodes))
			for _, n := range g.Nodes {
				systemOwnedByType[strcase.SnakeCase(n.Name)] = entx.HasAnnotation[entx.SystemOwnedSchema](n)
			}
...
{{- range $n := $.Nodes }}
	...
	{{- range $e := $n.Edges }}
		...
		"{{ toSnake $e.Name }}": {
			...
			HasSystemOwnedField: {{ isSystemOwned $e }},
		},
	{{- end }}
	},
	{{- end -}}
